...
namespace WindowsFormsApp1
{
    public partial class Form9 : Form
    {
        ...
        public void GenerateMap() // создаем карту
        {
            for (int i = 0; i < n * n; i++)
            {
                for (int j = 0; j < n * n; j++)
            ...
            for (int i = 0; i < 60; i++)
            {
                ShuffleMap(r.Next(0, 5)); //выбираемм рандомные числа и фасуем ячейки
            }
        ...
        public void HideCells() // создаем ф-ию которая будет скрывать рандомные ячейки массива
        {
            int N = 100;
            Random r = new Random();
            while (N > 0)
        ...
        public void ShuffleMap(int i)  // выбор действия над матрицей
        {
            switch (i)
            {
                case 0:
        ...
        public void SwapBlocksInColumn() // меняем блоки в приделах строк
        {
            Random r = new Random();
        ...
        public void SwapBlocksInRow() // меняем блоки в приделах столбцов
        {
            Random r = new Random();
            var block1 = r.Next(0, n);
            var block2 = r.Next(0, n);
        ...
        public void SwapRowsInBlock()  // рандомно меняем столбцы в приделе блока
        {
            Random r = new Random();
        ...
        public void SwapColumnsInBlock() // рандомно меняем строки в приделе блока
        {
            Random r = new Random();
            var block = r.Next(0, n); // выьираем рандомный блок
            var row1 = r.Next(0, n);
            var line1 = block * n + row1;
            var row2 = r.Next(0, n);
            while (row1 == row2)
        ...
        public void MatrixTransposition() //транспонируем матрицу
        {
        ...
        public void CreateMap() // создаем обЬекты на нашей карте
        {
            for (int i = 0; i < n * n; i++)
            {
                for (int j = 0; j < n * n; j++)
            ...
            public void OnCellPressed(object sender, EventArgs e) // оброботчик нажатия кнопки для выбора цифры 
            {
